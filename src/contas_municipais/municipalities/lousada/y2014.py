...
# Some pct values use 1 decimal place ("97,2", "11,8") — base _PT_NUM requires exactly 2.
...
def _parse_expenditure(text: str) -> list[dict]:
    ...
    sec = slice_section(text, "ORÇAMENTAL DA DESPESA", "PLANO")
    rows = []
...
def parse(files: dict[str, Path]) -> ParseResult:
    ...
    if pdf is None:
        raise FileNotFoundError("No PDF found for 2014 (expected key 'prestacao_contas' or 'other')")
